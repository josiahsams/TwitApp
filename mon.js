                ...
                if (contyp.match("image")) {
                    ...
                    requestImageSize(actualImgRef)
                        .then(size => {
                            ...
                            var paramList = { url:actualImgRef, width:size.width, height: size.height };
                            console.log(paramList);
                            request({url: "http://kinford3.aus.stglabs.ibm.com:5000/", qs: paramList}, function(err, response, body) {
                                    if (err) { console.log(err); return; }
                                    prediction = JSON.parse(body);
                                    console.log(prediction);
                                    myobj = { "user": tweet.user.name, "imgurl": actualImgRef,
